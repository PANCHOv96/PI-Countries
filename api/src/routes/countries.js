...
router.get('/', (req,res ,next) =>{
    const {name,activities,continents} = req.query;
    const conditions = {};
    ...
    if(name){
        ...
        countries.then(response => {
            ...
            if(!response.rows[0]){
                if(activities){
                    if(continents){
                        res.status(404).json({message:`There is no such Activity: ${activities} in the continent ${continents}`});
                    }else{
                        res.status(404).json({message:`There is no such Activity: ${activities}`});
                    }
                }
                else{
